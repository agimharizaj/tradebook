...
- Database: schema + RLS in `supabase/migrations/` (0001 init, 0002 import ext_id, 0003 notes, 0004 traded_on -> timestamptz + commission, 0005 chart_analyses, 0006 pair tags on notes/strategies, 0007 news_items headline archive). Apply 0004 before MT5 import, 0006 before pair tags, 0007 before the news archive fills.
- Pairs: master catalog in `src/lib/pairs.ts` (FX, metals, crypto, indices, energy); the user watchlist lives in auth metadata `pairs`, managed at Profile -> Trading pairs (`/profile/pairs`), read via `usePairs()`. It drives every pair dropdown (charts, risk, journal, analysis log, note/strategy tags). Add new instruments to the catalog only; sizing conventions live in `src/lib/risk.ts` (XAG 0.01/5000oz, crypto pip 1 contract 1 = coins; indices/energy not sizable). `/api/fx` covers fiat (Frankfurter), metals (gold-api), crypto (CoinGecko id map).
