...
- Landing page (`src/app/page.tsx`): no "free"/pricing/self-hosted selling language anywhere (owner's rule - this is not being sold to anyone). Nine feature cards; live demo components in `src/components/landing/`: HeroCurve, RiskDemo, EntryChecklist, JournalDemo (mini month calendar), SidekickDemo (scripted AI chat).
